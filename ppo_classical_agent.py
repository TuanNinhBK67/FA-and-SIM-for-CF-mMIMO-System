...
class PPOConfig:
    ...
    ent_coef: float = 5e-4 #1e-3
    vf_coef: float = 0.5
    max_grad_norm: float = 0.5
    target_kl: float | None = 0.015 #0.02
    ...
    ap_nums: int = 3
    antenna_nums: int = 4
    user_equipment_nums: int = 4
    layer_nums: int = 3
    num_elements_side: int = 5
...
def log_episode_statistics(infos, writer, global_step, episode_returns):
    """
    Compatible with vectorized Gymnasium info dictionaries produced from
    RecordEpisodeStatistics-wrapped sub-environments.
    """
...
def train(config: PPOConfig):
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        for step in range(config.num_steps):
            ...
            completed_returns = log_episode_statistics(
                infos,
                writer,
                global_step,
                episode_returns,
        ...
        if np.isfinite(mean_episode_return):
            writer.add_scalar(
                "charts/mean_episode_return_100",
                mean_episode_return,
                global_step,
            )
